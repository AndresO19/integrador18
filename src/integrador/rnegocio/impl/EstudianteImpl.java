...
import integrador.rnegocio.impl.EscuelaImpl.*;
import integrador.rnegocio.impl.NivelImpl.*;
    ...
    public static ArrayList<Estudiante> llenarEstudiantes(ConjuntoResultado rs) throws Exception {
        ...
        try {
            while (rs.next()) {
                ...
                estudiante = new Estudiante(rs.getInt("pcodigo"), rs.getString("pnombre"), rs.getString("pnombre"), rs.getString("pnombre"), rs.getInt("pcodigo"), PeriodoImpl.ObtenerPeriodoDadoCodigo(rs.getInt("pcodigo_periodo")), NivelImpl.ObtenerNivelDadoCodigo(rs.getInt("pcodigo_nivel")));
                lst.add(estudiante);
            }
        } catch (Exception e) {
            lst.clear();
